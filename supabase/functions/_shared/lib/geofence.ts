...
// Ported from server/lib/geofence.js — exported names unchanged.
//
...
export interface GeoSettings { floor: number; ceiling: number; trust: number; blockBuffer?: number }
...
export async function geoSettings(db: Db): Promise<GeoSettings> {
  const s: GeoSettings = { ...GEO_DEFAULTS };
  try {
...
//   }
export function evaluateGeofence(lat: any, lng: any, accuracy: any, geofences: any[], settings: GeoSettings = GEO_DEFAULTS): GeoDecision {
  const accRaw = +accuracy || 0;
  const acc = Math.min(Math.max(accRaw, settings.floor), settings.ceiling);
  let nearest: { dist: number; gf: any } = { dist: Infinity, gf: null };
  ...
  const nearestSite: string = nearest.gf?.site_name || "";
