...
// Ported from server/lib/businessHours.js — exported names unchanged.
//
...
export function businessHoursBetween(start: Date, end: Date): number {
  ...
  const cur = clampToBusinessMoment(start);
  let mins = 0;
  const stop = end;
  while (cur < stop) {
    ...
    mins += Math.max(0, (segmentEnd.getTime() - cur.getTime()) / 60000);
