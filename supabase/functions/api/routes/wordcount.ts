...
// Ported from server/routes/wordcount.js (Phase-3 Postgres version).
//
...
function countCharsInString(s: any): number {
...
function countCharsRecursive(value: any, parentKey?: string): number {
...
function rowCharCount(row: any): { chars: number; truncated: boolean } {
...
router.get("/", async (req, res) => {
  try {
    ...
    const params: any[] = [dateFrom + " 00:00:00", dateTo + " 23:59:59"];
    ...
    const rows = await pg.all(
      `SELECT id, user_id, user_name, action, entity_type, body_summary
        ...
        ORDER BY at DESC`, ...params);

    const byUser = new Map<any, any>();
...
router.get("/user-check/:user_id", async (req, res) => {
  try {
    ...
    const audit = await pg.get(
      `SELECT COUNT(*) total,
              MIN(at) first_action,
              MAX(at) last_action,
              COUNT(DISTINCT ip) distinct_ips,
              COUNT(DISTINCT LEFT(at,10)) active_days
         FROM audit_log WHERE user_id=?`, id);
    const ips = await pg.all(
      `SELECT ip, COUNT(*) c, MIN(at) first_seen, MAX(at) last_seen
         FROM audit_log
        WHERE user_id=? AND ip IS NOT NULL
     GROUP BY ip ORDER BY c DESC LIMIT 10`, id);
    const logins = await pg.all(
