...
//
// Ported from server/routes/payroll.js (Phase-3 Postgres version).
...
async function calculateForEmployee(settings: any, employee: any, month: string) {
  ...
  if (employee.salary_exempt) {
    ...
    return {
      ...
      late_marks: 0, lates_converted_absent: 0, late_penalty: 0, late_days: [] as any[],
      ...
      breakdown: [{ date: month + '-01', day: '—', label: 'salary_exempt', pay: 0, note: 'Flat monthly salary; daily breakdown not applicable' }] as any[],
    } as any;
  }
  ...
  if (!userId && employee.name) {
    ...
    if (nameMatch) {
      userId = nameMatch.id;
      try { await pg.run('UPDATE employees SET user_id = ? WHERE id = ?', userId, employee.id); } catch { /* ignore */ }
    }
  }
  ...
  const leaveByDate: Record<string, string> = {}; // date → leave_type ('casual','sick','earned','short_leave','comp_off')
  const shortLeaveByDate: Record<string, boolean> = {}; // date → true if short leave applied that day
  ...
  const breakdown: any[] = []; // per-day for slip
  const lateDays: any[] = []; // [{date, minutes_late, applies_penalty: bool}]
...
async function computeLeaveBalances(year: number) {
  const settings = await getSettings();
