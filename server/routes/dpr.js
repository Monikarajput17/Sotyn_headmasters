...
const getSetting = async (k) => (await pg.get('SELECT value FROM app_settings WHERE key=?', k))?.value ?? null;
...
router.get('/contractor-attendance', async (req, res) => {
  try {
...
router.get('/contractor-attendance/records', async (req, res) => {
  try {
...
router.post('/contractor-attendance', async (req, res) => {
  ...
  try {
    await pg.tx(async (t) => {
      await t.run('DELETE FROM contractor_attendance WHERE site_id=? AND attendance_date=?', site_id, date);
      for (const r of clean) {
        await t.run(`INSERT INTO contractor_attendance
          ...
          site_id, date, r.subcontractor_id || null, String(r.contractor_name).trim(),
...
async function dprCanSeeAll(user) {
...
// (incl. non-breaking space CHR(160), tabs, CR/LF) and quotes, then
...
const siteKeySql = (col) =>
  `UPPER(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(${col}, CHR(160), ''), CHR(9), ''), CHR(10), ''), CHR(13), ''), ' ', ''), '"', ''), CHR(39), ''))`;
...
router.get('/sites', async (req, res) => {
  try {
    ...
    const canSeeAll = await dprCanSeeAll(req.user);
    ...
    let sql = `SELECT MIN(s.id) as id,
      MIN(TRIM(REPLACE(REPLACE(REPLACE(REPLACE(s.name, '"', ''), CHR(39), ''), '  ', ' '), '  ', ' '))) as name,
...
router.post('/sites', async (req, res) => {
  try {
    ...
    let bbId = null;
    if (po_id) {
      const poRow = await pg.get('SELECT business_book_id FROM purchase_orders WHERE id=?', po_id);
      if (poRow?.business_book_id) bbId = poRow.business_book_id;
    }
    if (!bbId && name) {
      const byProject = await pg.get(`SELECT id FROM business_book
    ...
    const r = await pg.run('INSERT INTO sites (name, address, client_name, po_id, business_book_id, site_engineer_id, supervisor) VALUES (?,?,?,?,?,?,?)',
      name, address, client_name, po_id, bbId, site_engineer_id, supervisor);
  ...
  try {
    ...
    await pg.run(
      `UPDATE sites SET
       ...
       WHERE id=?`,
      name, address, client_name, site_engineer_id, supervisor, supervisor_id || null, status, req.params.id);
    ...
    if (status) {
      try {
        const cur = await pg.get('SELECT name FROM sites WHERE id=?', req.params.id);
        if (cur?.name) {
          await pg.run(
            `UPDATE sites SET status=?
              WHERE id != ?
                AND ${siteKeySql('name')} = ${siteKeySql('?')}`,
            status, req.params.id, cur.name);
        }
...
router.get('/sites/:site_id/ta-da-cost', async (req, res) => {
  try {
    const site = await pg.get('SELECT id, name FROM sites WHERE id=?', req.params.site_id);
    ...
    let dateClause = '';
    if (req.query.date) {
      dateClause = ' AND (required_by_date = ? OR LEFT(created_at,10) = ?)';
...
router.get('/sites/:site_id/staff-cost', async (req, res) => {
  try {
    const site = await pg.get('SELECT id, name, po_id, business_book_id FROM sites WHERE id=?', req.params.site_id);
    ...
    const pos = await pg.all(
      ...
      site.po_id, site.business_book_id);
    ...
    const engUsers = await pg.all(`SELECT id, name, email FROM users WHERE id IN (${placeholders})`, ...idList);
    const allEmployees = await pg.all(
    ...
    if (date) {
      for (const s of staffList) {
        const a = await pg.get('SELECT status, punch_in_time, admin_marked FROM attendance WHERE user_id=? AND date=?', s.user_id, date);
...
router.get('/sites/:site_id/po-items', async (req, res) => {
  try {
    const site = await pg.get('SELECT id, name, po_id, business_book_id FROM sites WHERE id=?', req.params.site_id);
    ...
    const sameNameBB = await pg.all(`SELECT DISTINCT s.business_book_id FROM sites s
                                      WHERE s.name = ? AND s.business_book_id IS NOT NULL`, site.name);

    // Path 3: site.po_id → purchase_orders.business_book_id.
    let poBbId = null;
    if (site.po_id) {
      const poRow = await pg.get('SELECT business_book_id FROM purchase_orders WHERE id=?', site.po_id);
    ...
    const bbByProject = await pg.all(`SELECT id FROM business_book WHERE TRIM(LOWER(project_name)) = TRIM(LOWER(?))`, site.name);
    ...
    const siteIds = (await pg.all('SELECT id FROM sites WHERE name=?', site.name)).map(r => r.id);
    ...
    for (const it of items) {
      const filledRow = await pg.get(filledSql, ...sidArgs, it.id, it.description);
      const filledQty = filledRow?.total || 0;
      const remaining = Math.max(0, (it.quantity || 0) - filledQty);
      result.push({ ...it, filled_qty: filledQty, remaining_qty: remaining });
    }
...
router.get('/summary', async (req, res) => {
  try {
    ...
    const activeSites = await pg.get(
      `SELECT COUNT(DISTINCT ${siteKeySql('name')}) as c FROM sites WHERE status='active'`
    );
    const todayDprs = await pg.get('SELECT COUNT(*) as c FROM dpr WHERE report_date=?', today);
    const pendingApproval = await pg.get("SELECT COUNT(*) as c FROM dpr WHERE approval_status='pending'");
    const billingReady = await pg.get('SELECT COUNT(*) as c FROM dpr WHERE billing_ready=1');
    const uid = req.user.id;
    const canSeeAll = await dprCanSeeAll(req.user);
    ...
    const missingSites = await pg.all(missingSql, ...missingParams);
    // date('now','-7 days') computed in JS (UTC, exact parity with SQLite).
    const sevenAgo = new Date(Date.now() - 7 * 86400000).toISOString().slice(0, 10);
    const variance = await pg.all(`SELECT d.report_date, s.name as site_name,
      ...
      WHERE d.report_date >= ? GROUP BY d.id, s.name ORDER BY d.report_date DESC LIMIT 20`, sevenAgo);
...
//
// (The idempotent ALTER TABLE guards that used to live here are gone —
// the Postgres schema is fully migrated, all columns exist.)
...
// Fire-and-forget — idempotent, and must not block module load.
pg.run(
...
router.get('/week-view', requirePermission('dpr', 'view'), async (req, res) => {
  try {
...
router.post('/plan-week', requirePermission('dpr', 'create'), async (req, res) => {
      ...
      for (const d of days) {
        ...
        if (!desc && cleanedItems.length) {
          ...
          for (const it of cleanedItems) {
            ...
            const pi = await t.get('SELECT description, unit, quantity, rate FROM po_items WHERE id = ?', it.po_item_id);
            if (!pi) continue;
            parts.push(`${pi.description}${it.planned_qty > 0 ? ` · ${it.planned_qty} ${pi.unit || ''}`.trim() : ''}`);
          }
          desc = parts.join(' | ') || null;
        }
        ...
        const existing = await t.get('SELECT id FROM dpr WHERE site_id = ? AND report_date = ?', site_id, date);
        let dprId;
        if (existing) {
          await t.run(`
            ...
            WHERE id = ?
          `, desc, mp, cost, req.user.id, existing.id);
          dprId = existing.id;
          out.updated++;
        } else {
          const r = await t.run(`
          ...
          `, site_id, date, req.user.id, desc, mp, cost, req.user.id);
          dprId = r.lastInsertRowid;
          out.created++;
        }

        // Replace the items list for this day.  We REPLACE all items for
        // the (dpr_id) on each save so re-saving the planning modal doesn't
        // accumulate ghosts.  Existing actuals (actual_qty) on rows already
        // filled would be wiped — to avoid that, the daily-submit endpoint
        // updates actual_qty in place rather than going through plan-week.
        // Carry rate from BOQ so the dpr_work_items row has enough context
        // for later reporting.
        await t.run('DELETE FROM dpr_work_items WHERE dpr_id = ?', dprId);
        for (const it of cleanedItems) {
          const pi = await t.get('SELECT description, unit, quantity, rate FROM po_items WHERE id = ?', it.po_item_id);
          if (!pi) continue;
          await t.run(`
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
          `, dprId, it.po_item_id, pi.description, pi.unit || 'nos', +pi.quantity || 0, +pi.rate || 0, it.planned_qty);
        }
...
router.post('/', async (req, res) => {
  ...
  let dprId;
  const existing = await pg.get('SELECT id, is_planned_template FROM dpr WHERE site_id = ? AND report_date = ?', site_id, report_date);
  if (existing) {
    await pg.run(`UPDATE dpr SET
      ...
      WHERE id = ?`,
      req.user.id, weather || 'clear', overall_status || 'on_track',
    ...
    const r = await pg.run(`INSERT INTO dpr (site_id, report_date, submitted_by, submission_time, weather, overall_status,
      ...
      site_id, report_date, req.user.id, weather || 'clear', overall_status || 'on_track',
      shift || 'day', contractor_name, contractor_manpower || 0, mb_sheet_no,
      grand_total_a || 0, grand_total_b || 0, profit_loss || 0,
      floor_zone, system_type, safety_toolbox_talk ? 1 : 0, safety_ppe_compliance ? 1 : 0,
      safety_incidents, next_day_plan, hindrances, hindrance_category || null, remarks);
  ...
  for (const c of (contractors || [])) {
    ...
    if (!name && !mp) continue;
    await pg.run('INSERT INTO dpr_contractors (dpr_id, name, manpower) VALUES (?,?,?)', dprId, name || null, mp);
  }
  ...
  for (const w of (work_items || [])) {
    ...
    const validPoItemId = w.po_item_id
      ? ((await pg.get('SELECT id FROM po_items WHERE id=?', w.po_item_id)) ? w.po_item_id : null)
      : null;
    ...
    const validWoId = w.work_order_id
      ? ((await pg.get('SELECT id FROM proj_work_orders WHERE id=?', +w.work_order_id)) ? +w.work_order_id : null)
      : null;
    await pg.run(insertWorkSql,
  ...
  for (const c of (manpower || [])) {
    ...
    if (costType) await pg.run('INSERT INTO dpr_manpower (dpr_id, trade, required, deployed, shortage) VALUES (?,?,?,?,?)', dprId, costType, qty, rate, amount);
  }

  // Machinery/Tools
  for (const mc of (machinery || [])) {
    if (mc.equipment) await pg.run('INSERT INTO dpr_machinery (dpr_id, equipment, quantity, hours_used, condition, remarks) VALUES (?,?,?,?,?,?)',
      dprId, mc.equipment, mc.quantity || 1, mc.hours_used || 0, mc.condition || 'working', mc.remarks);
  }
  ...
  for (const m of (materials || [])) {
    ...
    const validPoItemId = m.po_item_id ? ((await pg.get('SELECT id FROM po_items WHERE id=?', m.po_item_id)) ? m.po_item_id : null) : null;
    await pg.run(insertMatSql,
    ...
    if (consumed > 0 && m.item_master_id && siteStore?.id) {
      try {
        const cur = await pg.get('SELECT * FROM stock_balance WHERE warehouse_id=? AND item_master_id=?', siteStore.id, m.item_master_id);
        const prevQty = cur ? +cur.quantity : 0;
        if (prevQty >= consumed) {
          const rate = cur ? +cur.avg_rate : 0;
          await pg.run('UPDATE stock_balance SET quantity=?, updated_at=CURRENT_TIMESTAMP WHERE id=?',
            prevQty - consumed, cur.id);
          await pg.run(
            `INSERT INTO stock_movements
              (warehouse_id, item_master_id, type, quantity, rate, total_value,
               reference_type, reference_id, site_id, notes, created_by)
             VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
            siteStore.id, m.item_master_id, 'OUT', consumed, rate, consumed * rate,
...
async function consecutiveLossDays(siteId, asOfDate) {
  ...
  const days = await pg.all(`SELECT report_date AS d, SUM(profit_loss) AS pl
                             FROM dpr WHERE site_id=? AND report_date<=?
                             GROUP BY report_date ORDER BY report_date DESC LIMIT 30`, siteId, asOfDate);
  let streak = 0;
  let cursor = asOfDate;
...
async function checkConsecutiveLossAndAlert(latestDprId, siteId) {
  const latest = await pg.get(`SELECT d.id, d.report_date, d.profit_loss,
                                      s.name AS site_name, s.client_name
                               FROM dpr d LEFT JOIN sites s ON d.site_id=s.id
                               WHERE d.id=?`, latestDprId);
  if (!latest) return;

  const streak = await consecutiveLossDays(siteId, latest.report_date);
  if (streak < 3) return;
  ...
  const lastAlertedFor = (await pg.get('SELECT MAX(streak_alert_sent_for) AS d FROM dpr WHERE site_id=?', siteId)).d;
  if (lastAlertedFor && lastAlertedFor >= latest.report_date) return;

  // Pull the 3 most recent loss rows for context.
  const recent = await pg.all(`SELECT report_date, profit_loss, hindrance_category, hindrances
                               FROM dpr WHERE site_id=?
                               ORDER BY report_date DESC LIMIT 5`, siteId);
  ...
  if (result?.sent) {
    await pg.run('UPDATE dpr SET streak_alert_sent_for=? WHERE id=?', latest.report_date, latest.id);
  ...
  try {
    const eng = await pg.get(
      ...
      siteId);
    let director = null;
    try { director = require('../lib/email').getDirectorEmail(); } catch {}
...
router.get('/loss-dashboard', async (req, res) => {
  try {
    ...
    const rows = await pg.all(`SELECT d.id, d.site_id, d.report_date, d.profit_loss, d.grand_total_a,
    ...
    for (const r of rows) {
      r.consecutive_loss_days = await consecutiveLossDays(r.site_id, r.report_date);
    }
...
// (mam, 2026-05-16: "on address click proof so that problem can
// solve and identify")
router.patch('/:id/loss-addressed', async (req, res) => {
  try {
    const { addressed, note, proof_url } = req.body || {};
    const next = addressed ? 1 : 0;
    const r = await pg.run(`UPDATE dpr SET loss_addressed=?, loss_addressed_by=?, loss_addressed_at=?, loss_addressed_note=?, loss_addressed_proof_url=?
                            WHERE id=?`,
      next,
...
router.get('/engineer-compliance', async (req, res) => {
  try {
    const uid = req.user.id;
    const canSeeAll = await dprCanSeeAll(req.user);
    ...
    let engineers = await pg.all(`
      SELECT u.id, u.name, u.email, STRING_AGG(r.name, ',') AS role_names
        FROM users u
    ...
    if (allSiteIds.length && allEngIds.length) {
      ...
      const allActive = await pg.all(`SELECT id, name FROM sites WHERE status = 'active'`);
      ...
      `, ...expandedSiteIds, ...allEngIds, from, to);
    ...
    if (allEngIds.length) {
      ...
      const engContractorMp = await pg.all(`
...
router.get('/:id', async (req, res) => {
  try {
    const dpr = await pg.get(`SELECT d.*, s.name as site_name, s.client_name, u.name as submitted_by_name
      FROM dpr d LEFT JOIN sites s ON d.site_id=s.id LEFT JOIN users u ON d.submitted_by=u.id WHERE d.id=?`, req.params.id);
    if (!dpr) return res.status(404).json({ error: 'Not found' });
    dpr.work_items = await pg.all('SELECT * FROM dpr_work_items WHERE dpr_id=?', req.params.id);
    dpr.manpower = await pg.all('SELECT * FROM dpr_manpower WHERE dpr_id=?', req.params.id);
    dpr.materials = await pg.all('SELECT * FROM dpr_material WHERE dpr_id=?', req.params.id);
    dpr.machinery = await pg.all('SELECT * FROM dpr_machinery WHERE dpr_id=?', req.params.id);
    dpr.contractors = await pg.all('SELECT id, name, manpower FROM dpr_contractors WHERE dpr_id=? ORDER BY id', req.params.id);
    res.json(dpr);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.put('/:id/approve', requirePermission('dpr', 'approve'), async (req, res) => {
  try {
    const { approval_status, billing_ready } = req.body;
    await pg.run('UPDATE dpr SET approval_status=?, billing_ready=?, approved_by=? WHERE id=?',
      approval_status, billing_ready ? 1 : 0, req.user.id, req.params.id);
    if (billing_ready) {
      const dpr = await pg.get('SELECT d.*, s.client_name, s.name as site_name FROM dpr d JOIN sites s ON d.site_id=s.id WHERE d.id=?', req.params.id);
      if (dpr?.client_name) {
        const existing = await pg.get('SELECT id FROM receivables WHERE client_name=? AND project_name=? AND invoice_date=?', dpr.client_name, dpr.site_name, dpr.report_date);
        if (!existing) {
          await pg.run('INSERT INTO receivables (client_name, project_name, invoice_date, invoice_amount, outstanding_amount, due_date, status, created_by) VALUES (?,?,?,0,0,?,?,?) ON CONFLICT DO NOTHING',
            dpr.client_name, dpr.site_name, dpr.report_date, dpr.report_date, 'green', req.user.id);
        }
      }
    }
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    await pg.run('DELETE FROM dpr_work_items WHERE dpr_id=?', id);
    await pg.run('DELETE FROM dpr_manpower WHERE dpr_id=?', id);
    await pg.run('DELETE FROM dpr_material WHERE dpr_id=?', id);
    await pg.run('DELETE FROM dpr_machinery WHERE dpr_id=?', id);
    await pg.run('DELETE FROM dpr WHERE id=?', id);
...
router.delete('/sites/:id', async (req, res) => {
  try {
    const id = req.params.id;
    const dprCount = (await pg.get('SELECT COUNT(*) as c FROM dpr WHERE site_id=?', id)).c;
    if (dprCount > 0) return res.status(409).json({ error: 'Cannot delete: DPRs reference this site' });
    await pg.run('DELETE FROM sites WHERE id=?', id);
    res.json({ message: 'Deleted' });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
async function progressHandler(req, res) {
  try {
    ...
    // We resolve the engineer→site assignment via two paths so the DB
    ...
    const sitesPerEng = await pg.all(`
      SELECT
        e.id   AS engineer_id,
        MIN(s.id) AS site_id,
        s.name AS site_name,
    ...
    const sitesByName = await pg.all(`
    ...
    const poItems = allBbIds.length
      ? await pg.all(`SELECT id, business_book_id, description, unit, rate, quantity FROM po_items WHERE business_book_id IN (${allBbIds.map(()=>'?').join(',')})`, ...allBbIds)
      : [];
...
router.get('/payment-check/:site_id', async (req, res) => {
  try {
    const today = new Date().toISOString().split('T')[0];
    const dpr = await pg.get('SELECT id FROM dpr WHERE site_id=? AND report_date=?', req.params.site_id, today);
