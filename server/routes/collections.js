...
async function raisePmsTaskFromRemark(db, r, remark, opts = {}) {
  ...
  const aanchal = await db.get("SELECT id FROM users WHERE COALESCE(active,1)=1 AND LOWER(TRIM(name))='aanchal' LIMIT 1")
    || await db.get("SELECT id FROM users WHERE COALESCE(active,1)=1 AND LOWER(name) LIKE 'aanchal%' ORDER BY id LIMIT 1");
  ...
  const projectId = (r.site_id && await db.get('SELECT 1 FROM business_book WHERE id=?', r.site_id)) ? r.site_id : null;
  ...
  const info = await db.run(
    `INSERT INTO pms_tasks (title, description, project_id, project_name_snapshot, crm_name, assigned_by, assigned_to, due_date)
       VALUES (?,?,?,?,?,?,?,?)`,
    title, desc, projectId, r.site_name || r.client_name || null, r.crm_name || null, opts.loggedBy, assignee, opts.promised_date || null);
...
router.get('/', async (req, res) => {
  try {
                ...
                WHERE p.project_id = r.site_id
                   OR (r.site_name IS NOT NULL AND r.site_name <> ''
                       AND (p.project_name_snapshot = r.site_name
                            OR p.project_name_snapshot ILIKE '%' || r.site_name || '%'))
             ) as pms_tasks_count
        FROM receivables r
    ...
    if (client) { sql += ' AND (r.client_name ILIKE ? OR r.site_name ILIKE ?)'; params.push(`%${client}%`, `%${client}%`); }
    if (search) { sql += ' AND (r.client_name ILIKE ? OR r.site_name ILIKE ? OR r.invoice_number ILIKE ?)'; params.push(`%${search}%`, `%${search}%`, `%${search}%`); }
    ...
    if (ids.length > 0) {
      ...
      const allPayments = await pg.all(
...
router.get('/md-dashboard', async (req, res) => {
  try {
    ...
    // Postgres needs the per-site aggregation done in an inner grouped
    // subquery (g) — the correlated activity subqueries then key off the
    // grouped site row instead of a bare ungrouped column (which SQLite
    // tolerated but PG rejects).
    const rows = await pg.all(`
      SELECT
        g.site_name, g.site_id, g.crm_name, g.owner_name, g.invoice_count,
        g.target, g.received, g.outstanding, g.oldest_ageing,
        g.next_planned_date, g.last_discussion,
        (SELECT COUNT(*) FROM pms_tasks p
           WHERE (g.site_id IS NOT NULL AND p.project_id = g.site_id)
              OR (g.site_name IS NOT NULL AND g.site_name <> ''
                  AND (p.project_name_snapshot = g.site_name
                       OR p.project_name_snapshot ILIKE '%' || g.site_name || '%'))
        )                                     as pms_tasks_count,
        (SELECT COUNT(*) FROM location_tracking lt
           WHERE lt.time >= ?
             AND lt.site_name IS NOT NULL AND lt.site_name <> 'Outside'
             AND lt.site_name = g.site_name
        )                                     as location_pings_7d,
        (SELECT MAX(cf.follow_up_date) FROM collection_follow_ups cf
           JOIN receivables r2 ON r2.id = cf.receivable_id
          WHERE COALESCE(r2.site_name, r2.client_name) = g.site_name
        )                                     as last_follow_up,
        ...
        (SELECT COUNT(*) FROM indents ind
           WHERE ind.site_name = g.site_name
              OR ind.client_name = g.site_name
        )                                     as indents_count,
        (SELECT COUNT(*) FROM indents ind
           WHERE (ind.site_name = g.site_name
                  OR ind.client_name = g.site_name)
  ...
  try {
    ...
    const rows = await pg.all(`
      SELECT MIN(bb.id) as business_book_id,
             ...
             STRING_AGG(DISTINCT bb.client_name, ',')  as client_names,
             STRING_AGG(DISTINCT bb.project_name, ',') as project_names,
    ...
    // Per-row PO lookups (one extra SELECT per company; cheap at our row counts)
    const crmSql = `
    ...
    const enrich = async (companyName) => {
      if (!companyName) return { crm_name: null, latest_po_value: 0, latest_po_number: null };
      return {
        crm_name: (await pg.get(crmSql, companyName))?.crm_name || null,
        latest_po_value: (await pg.get(totalSql, companyName))?.total || 0,
        latest_po_number: (await pg.get(latestPoSql, companyName))?.po_number || null,
      };
    };

    const list = [];
    for (const r of rows) {
      const enr = await enrich(r.company_name);
      list.push({
...
router.get('/summary', async (req, res) => {
  try {
    const total = await pg.get('SELECT COALESCE(SUM(outstanding_amount),0) as total FROM receivables WHERE outstanding_amount > 0');
    const byBucket = await pg.all('SELECT ageing_bucket, COUNT(*) as count, COALESCE(SUM(outstanding_amount),0) as total FROM receivables WHERE outstanding_amount > 0 GROUP BY ageing_bucket');
    const byStatus = await pg.all('SELECT status, COUNT(*) as count, COALESCE(SUM(outstanding_amount),0) as total FROM receivables WHERE outstanding_amount > 0 GROUP BY status');
    const topClients = await pg.all('SELECT client_name, SUM(outstanding_amount) as total FROM receivables WHERE outstanding_amount > 0 GROUP BY client_name ORDER BY total DESC LIMIT 10');
    const overdue = await pg.get('SELECT COUNT(*) as count, COALESCE(SUM(outstanding_amount),0) as total FROM receivables WHERE ageing_days > 30 AND outstanding_amount > 0');
...
router.get('/payment-advice', async (req, res) => {
  try {
    ...
    const rows = bbid
      ? await pg.all(`SELECT * FROM receivables WHERE business_book_id = ? ORDER BY COALESCE(invoice_date, created_at)`, bbid)
      : await pg.all(`SELECT * FROM receivables WHERE LOWER(TRIM(client_name)) = LOWER(TRIM(?)) ORDER BY COALESCE(invoice_date, created_at)`, clientName);
    ...
    try {
      ...
      let bb = null;
      if (resolveBbid) bb = await pg.get(`SELECT company_name, client_name, billing_address, gstin, state FROM business_book WHERE id = ?`, resolveBbid);
      if (!bb && clientName) bb = await pg.get(`SELECT company_name, client_name, billing_address, gstin, state FROM business_book WHERE LOWER(TRIM(COALESCE(client_name,''))) = LOWER(TRIM(?)) OR LOWER(TRIM(COALESCE(company_name,''))) = LOWER(TRIM(?)) ORDER BY id DESC LIMIT 1`, clientName, clientName);
...
router.post('/', requirePermission('collections', 'create'), async (req, res) => {
  try {
    const b = req.body || {};
    const {
    ...
    try {
      if (String(last_discussion || '').trim()) {
        const row = await pg.get('SELECT * FROM receivables WHERE id=?', r.lastInsertRowid);
        pms_task_id = await raisePmsTaskFromRemark(pg, row, last_discussion, {
...
router.put('/:id', requirePermission('collections', 'edit'), async (req, res) => {
  try {
    const cur = await pg.get('SELECT * FROM receivables WHERE id=?', req.params.id);
    ...
    // triggers "FOREIGN KEY constraint failed" because the row's old
    ...
    const userExists = async (id) => {
      ...
      return !!(await pg.get('SELECT 1 FROM users WHERE id=?', v));
    };
    const siteExists = async (id) => {
      const v = +id; if (!Number.isFinite(v) || v <= 0) return false;
      return !!(await pg.get('SELECT 1 FROM sites WHERE id=?', v));
    };

    if (b.owner_id !== undefined)          set('owner_id', (await userExists(b.owner_id)) ? +b.owner_id : null);
    ...
    if (b.site_id !== undefined)           set('site_id', (await siteExists(b.site_id)) ? +b.site_id : null);
    ...
    await pg.run(`UPDATE receivables SET ${sets.join(', ')} WHERE id=?`, ...params);
    ...
    try {
      ...
      if (noteChanged || discChanged) {
        const r = await pg.get('SELECT * FROM receivables WHERE id=?', req.params.id) || cur;
        const remark = discChanged ? b.last_discussion : b.follow_up_notes;
        pms_task_id = await raisePmsTaskFromRemark(pg, r, remark, {
...
router.delete('/:id', requirePermission('collections', 'delete'), async (req, res) => {
  try {
    const id = req.params.id;
    const received = (await pg.get('SELECT COUNT(*) as c FROM collections WHERE receivable_id=?', id)).c;
    if (received > 0) return res.status(409).json({ error: 'Cannot delete: collections have been recorded against this receivable' });
    await pg.run('DELETE FROM collection_follow_ups WHERE receivable_id=?', id);
    await pg.run('DELETE FROM receivables WHERE id=?', id);
    res.json({ message: 'Deleted' });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// Add follow-up
router.post('/:id/follow-up', requirePermission('collections', 'edit'), async (req, res) => {
  try {
    const { follow_up_date, contact_method, response, promised_date, promised_amount } = req.body;
    await pg.run('INSERT INTO collection_follow_ups (receivable_id, follow_up_date, contact_method, response, promised_date, promised_amount, followed_by) VALUES (?,?,?,?,?,?,?)',
      req.params.id, follow_up_date, contact_method, response, promised_date, promised_amount, req.user.id);

    // Update receivable follow-up status
    await pg.run('UPDATE receivables SET follow_up_status=?, follow_up_date=?, follow_up_notes=?, updated_at=CURRENT_TIMESTAMP WHERE id=?',
      'contacted', follow_up_date, response, req.params.id);
    ...
    let pms_task_id = null;
    try {
      const r = await pg.get('SELECT * FROM receivables WHERE id=?', req.params.id) || {};
      pms_task_id = await raisePmsTaskFromRemark(pg, r, response, {
...
router.get('/:id/follow-ups', async (req, res) => {
  try {
    const followUps = await pg.all('SELECT f.*, u.name as followed_by_name FROM collection_follow_ups f LEFT JOIN users u ON f.followed_by=u.id WHERE f.receivable_id=? ORDER BY f.created_at DESC', req.params.id);
    res.json(followUps);
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// Record collection (payment received from client)
router.post('/:id/collect', requirePermission('collections', 'edit'), async (req, res) => {
  try {
    ...
    const rec = await pg.get('SELECT * FROM receivables WHERE id=?', req.params.id);
    ...
    await pg.run('INSERT INTO collections (receivable_id, amount, collection_date, payment_mode, transaction_ref, notes, collected_by) VALUES (?,?,?,?,?,?,?)',
      req.params.id, amt, collection_date || new Date().toISOString().split('T')[0], payment_mode || null, transaction_ref || null, notes || null, req.user.id);
    ...
    await pg.run('UPDATE receivables SET received_amount=?, outstanding_amount=?, ageing_days=?, ageing_bucket=?, status=?, updated_at=CURRENT_TIMESTAMP WHERE id=?',
      newReceived, Math.max(0, newOutstanding), days, bucket, statusColor, req.params.id);
    ...
    const billSync = await syncSalesBillPaymentStatus(pg, req.params.id);
    ...
    const dailyRes = await ensureTodayCashFlowDaily(pg, today);
    const daily = { id: dailyRes.id };
    await pg.run('INSERT INTO cash_flow_entries (daily_id, date, type, category, description, amount, payment_mode, party_name, reference_type, reference_id, created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?)',
      daily.id, today, 'inflow', 'Collection', `Collection from ${rec.client_name} - ${rec.invoice_number || ''}`, amount, payment_mode, rec.client_name, 'collection', req.params.id, req.user.id);
...
router.post('/refresh-ageing', requirePermission('collections', 'edit'), async (req, res) => {
  try {
