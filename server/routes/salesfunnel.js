...
router.get('/', requirePermission('leads', 'view'), async (req, res) => {
  try {
    ...
    if (search) {
      sql += ' AND (client_name ILIKE ? OR company_name ILIKE ? OR lead_no ILIKE ? OR phone ILIKE ?)';
      params.push(`%${search}%`, `%${search}%`, `%${search}%`, `%${search}%`);
    }
    sql += ' ORDER BY created_at DESC';
    res.json(withSla(await pg.all(sql, ...params)));
...
router.get('/dashboard', requirePermission('leads', 'view'), async (req, res) => {
  try {
    const total = await pg.get('SELECT COUNT(*) as c FROM sales_funnel');
    const bystage = await pg.all('SELECT current_stage, COUNT(*) as count FROM sales_funnel GROUP BY current_stage');
    ...
    // migration.
    const won = await pg.get("SELECT COUNT(*) as c, COALESCE(SUM(won_amount),0) as amount FROM sales_funnel WHERE current_stage IN ('contract_signed','won')");
    const lost = await pg.get("SELECT COUNT(*) as c FROM sales_funnel WHERE current_stage='lost'");
    // First of the current month (UTC) — SQLite's date('now','start of month').
    const monthStart = new Date().toISOString().slice(0, 7) + '-01';
    const thisMonth = await pg.get("SELECT COUNT(*) as c FROM sales_funnel WHERE created_at >= ?", monthStart);
    const byCategory = await pg.all("SELECT category, COUNT(*) as count FROM sales_funnel WHERE category IS NOT NULL AND category != '' GROUP BY category");
    const bySC = await pg.all("SELECT assigned_sc, COUNT(*) as count FROM sales_funnel WHERE assigned_sc IS NOT NULL AND assigned_sc != '' GROUP BY assigned_sc");
    const recent = await pg.all('SELECT * FROM sales_funnel ORDER BY updated_at DESC LIMIT 10');
    const today = new Date().toISOString().split('T')[0];
    let todayFollowups = 0, overdueFollowups = 0;
    try {
      todayFollowups = (await pg.get("SELECT COUNT(*) as c FROM lead_followups WHERE done=0 AND followup_date=?", today))?.c || 0;
      overdueFollowups = (await pg.get("SELECT COUNT(*) as c FROM lead_followups WHERE done=0 AND followup_date<?", today))?.c || 0;
    } catch(e) {}
...
router.get('/:id', requirePermission('leads', 'view'), async (req, res) => {
  try {
    const lead = await pg.get('SELECT * FROM sales_funnel WHERE id=?', req.params.id);
...
async function audit(db, lead_id, stage, action, user, opts = {}) {
  try {
    await db.run(`
      INSERT INTO sales_funnel_audit (lead_id, stage, action, actor_id, actor_name, evidence_url, notes)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `, lead_id, stage || null, action, user?.id || null, user?.name || null, opts.evidence_url || null, opts.notes || null);
  } catch {}
}
...
router.post('/', requirePermission('leads', 'create'), async (req, res) => {
  try {
    ...
    const leadNo = await nextSequencePg(pg, 'sales_funnel', 'lead_no', 'SEPL', { startFrom: 9000, pad: 4 });
    ...
    const r = await pg.run(`INSERT INTO sales_funnel
      ...
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'new_lead', CURRENT_TIMESTAMP)`,
    ...
    await audit(pg, r.lastInsertRowid, 'new_lead', 'create', req.user, {
...
// is audited.
router.post('/:id/drop', requirePermission('leads', 'edit'), async (req, res) => {
  try {
    const { reason } = req.body;
    if (!reason || !String(reason).trim()) return res.status(400).json({ error: 'Drop reason is required' });
    const cur = await pg.get('SELECT id, current_stage, dropped FROM sales_funnel WHERE id=?', req.params.id);
    ...
    await audit(pg, cur.id, cur.current_stage, 'drop', req.user, { notes: reason });
    res.json({ message: 'Lead dropped' });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// GET audit log for a lead — read-only timeline for the audit panel.
router.get('/:id/audit', requirePermission('leads', 'view'), async (req, res) => {
  try {
    const rows = await pg.all(`
...
router.put('/:id', requirePermission('leads', 'edit'), async (req, res) => {
  try {
    ...
    if (errors.length) return res.status(400).json({ error: errors.join(' · ') });
    const cur = await pg.get('SELECT current_stage FROM sales_funnel WHERE id=?', req.params.id);
    if (!cur) return res.status(404).json({ error: 'Lead not found' });
    ...
    await audit(pg, req.params.id, cur.current_stage, 'edit', req.user, { notes: 'Stage 1 fields updated' });
...
router.post('/:id/stage', requirePermission('leads', 'edit'), async (req, res) => {
  try {
    ...
    if (stage === 'boq_costing' && (b.boq_file_link || +b.boq_amount > 0)) {
      try {
        await pg.run(`INSERT INTO sales_funnel_boqs (funnel_id, boq_file_link, boq_amount, created_by, notes)
                      VALUES (?, ?, ?, ?, ?)`,
          req.params.id, b.boq_file_link || null, +b.boq_amount || 0, b.boq_created_by || req.user.name, b.boq_notes || null);
      } catch (_) { /* history is best-effort */ }
    }
    await audit(pg, req.params.id, stage, 'enter_stage', req.user, {
...
router.get('/:id/boqs', requirePermission('leads', 'view'), async (req, res) => {
  try {
    const rows = await pg.all(
...
router.post('/:id/boq', requirePermission('leads', 'edit'), async (req, res) => {
  const b = req.body || {};
  ...
  try {
    await pg.run(`INSERT INTO sales_funnel_boqs (funnel_id, boq_file_link, boq_amount, created_by, notes)
                  VALUES (?, ?, ?, ?, ?)`,
      req.params.id, b.boq_file_link || null, +b.boq_amount || 0, req.user.name, b.boq_notes || null);
    await pg.run(`UPDATE sales_funnel SET boq_file_link=?, boq_amount=?, boq_date=CURRENT_TIMESTAMP, updated_at=CURRENT_TIMESTAMP WHERE id=?`,
      b.boq_file_link || null, +b.boq_amount || 0, req.params.id);
    try { await audit(pg, req.params.id, 'boq_costing', 'add_boq', req.user, { notes: b.boq_notes || null }); } catch (_) {}
    res.json({ ok: true });
  } catch (e) {
...
router.get('/:id/followups', requirePermission('leads', 'view'), async (req, res) => {
  try {
    res.json(await pg.all(`SELECT f.*, u.name as created_by_name, u2.name as done_by_name FROM lead_followups f
...
// POST add follow-up
router.post('/:id/followup', requirePermission('leads', 'create'), async (req, res) => {
  try {
    ...
    const r = await pg.run('INSERT INTO lead_followups (lead_id, followup_date, followup_time, type, notes, next_followup_date, created_by) VALUES (?,?,?,?,?,?,?)',
      req.params.id, followup_date, followup_time, type || 'call', notes, next_followup_date, req.user.id);
    res.status(201).json({ id: r.lastInsertRowid });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// PUT log follow-up outcome
router.put('/followup/:fid', requirePermission('leads', 'edit'), async (req, res) => {
  try {
    const { outcome, notes, next_followup_date } = req.body;
    if (!outcome) return res.status(400).json({ error: 'Outcome required' });
    await pg.run('UPDATE lead_followups SET outcome=?, notes=?, done=1, done_by=?, next_followup_date=? WHERE id=?',
      outcome, notes, req.user.id, next_followup_date, req.params.fid);
    // Auto-create next follow-up if set
    if (next_followup_date) {
      const fu = await pg.get('SELECT lead_id FROM lead_followups WHERE id=?', req.params.fid);
      if (fu) {
        await pg.run('INSERT INTO lead_followups (lead_id, followup_date, type, notes, created_by) VALUES (?,?,?,?,?)',
          fu.lead_id, next_followup_date, 'call', 'Auto-scheduled from previous follow-up', req.user.id);
      }
    }
...
router.get('/followups/today', requirePermission('leads', 'view'), async (req, res) => {
  try {
    const today = new Date().toISOString().split('T')[0];
    const pending = await pg.all(`SELECT f.*, sf.lead_no, sf.client_name, sf.company_name, sf.phone, sf.current_stage
      FROM lead_followups f JOIN sales_funnel sf ON f.lead_id=sf.id
      WHERE f.done=0 AND f.followup_date <= ? ORDER BY f.followup_date`, today);
    res.json(pending);
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// GET overdue follow-ups
router.get('/followups/overdue', requirePermission('leads', 'view'), async (req, res) => {
  try {
    const today = new Date().toISOString().split('T')[0];
    const overdue = await pg.all(`SELECT f.*, sf.lead_no, sf.client_name, sf.company_name, sf.phone
      FROM lead_followups f JOIN sales_funnel sf ON f.lead_id=sf.id
      WHERE f.done=0 AND f.followup_date < ? ORDER BY f.followup_date`, today);
    res.json(overdue);
  } catch (e) { res.status(500).json({ error: e.message }); }
...
router.delete('/:id', requirePermission('leads', 'delete'), async (req, res) => {
  try {
    await pg.run('DELETE FROM sales_funnel WHERE id=?', req.params.id);
