...
// LIST
router.get('/', requirePermission('snags', 'view'), async (req, res) => {
  try {
    ...
    if (search) {
      sql += ' AND (s.description ILIKE ? OR s.location ILIKE ? OR s.snag_no ILIKE ? OR s.site_name ILIKE ?)';
      const q = `%${search}%`;
      params.push(q, q, q, q);
    }
...
router.get('/stats', requirePermission('snags', 'view'), async (req, res) => {
  try {
    const total = (await pg.get('SELECT COUNT(*) as c FROM snags')).c;
    const open = (await pg.get("SELECT COUNT(*) as c FROM snags WHERE status='open'")).c;
    const submitted = (await pg.get("SELECT COUNT(*) as c FROM snags WHERE status='submitted'")).c;
    const approved = (await pg.get("SELECT COUNT(*) as c FROM snags WHERE status='approved'")).c;
    const rejected = (await pg.get("SELECT COUNT(*) as c FROM snags WHERE status='rejected'")).c;
    const critical = (await pg.get("SELECT COUNT(*) as c FROM snags WHERE priority='critical' AND status NOT IN ('approved')")).c;
    res.json({ total, open, submitted, approved, rejected, critical });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// RAISE
router.post('/', requirePermission('snags', 'create'), async (req, res) => {
  try {
    const b = req.body;
    ...
    const snagNo = await nextSequencePg(pg, 'snags', 'snag_no', `SNAG-${yr}-`, { startFrom: 0, pad: 4 });
    ...
    if (!assigneeName && b.assigned_to) {
      const u = await pg.get('SELECT name FROM users WHERE id=?', b.assigned_to);
...
router.put('/:id', requirePermission('snags', 'edit'), async (req, res) => {
  try {
    const b = req.body;
    const cur = await pg.get('SELECT raised_by, status FROM snags WHERE id=?', req.params.id);
    if (!cur) return res.status(404).json({ error: 'Snag not found' });
    if (cur.raised_by !== req.user.id && !(await isApprover(req.user))) {
    ...
    // FK columns: '' from a cleared dropdown must become NULL, never an
    // empty string (an '' FK value breaks the row).
    const fkFields = ['site_id','assigned_to'];
    const sets = []; const vals = [];
    for (const f of fields) if (b[f] !== undefined) {
      sets.push(`${f}=?`);
      vals.push(fkFields.includes(f) && b[f] === '' ? null : b[f]);
    }
    if (!sets.length) return res.status(400).json({ error: 'No fields to update' });
    vals.push(req.params.id);
    await pg.run(`UPDATE snags SET ${sets.join(', ')} WHERE id=?`, ...vals);
...
router.post('/:id/submit', async (req, res) => {
  try {
    ...
    const s = await pg.get('SELECT * FROM snags WHERE id=?', req.params.id);
    if (!s) return res.status(404).json({ error: 'Not found' });
    if (s.assigned_to !== req.user.id && !(await isApprover(req.user))) {
    ...
    await pg.run(`
      UPDATE snags
       ...
       WHERE id=?
    `, proof_url, proof_notes || null, req.user.id, req.params.id);
...
router.post('/:id/approve', async (req, res) => {
  try {
    const s = await pg.get('SELECT * FROM snags WHERE id=?', req.params.id);
    if (!s) return res.status(404).json({ error: 'Not found' });
    if (s.raised_by !== req.user.id && !(await isApprover(req.user))) {
...
router.delete('/:id', requirePermission('snags', 'delete'), async (req, res) => {
  try {
    await pg.run('DELETE FROM snags WHERE id=?', req.params.id);
