...
const pg = require('../db/pg');                      // one Postgres now holds erp + chat tables
...
const isMember = async (db, g, u) => !!(await db.get('SELECT 1 FROM chat_group_members WHERE group_id=? AND user_id=?', g, u));
...
const canAccess = async (db, req, g) => {
...
const userName = async (uid) => { try { return (await pg.get('SELECT name FROM users WHERE id=?', uid))?.name || ''; } catch { return ''; } };
const markRead = async (db, g, uid, knownMax) => {
...
async function enrichGroups(db, uid, groups, { withMembers = true } = {}) {
...
async function pageOfGroups(db, { uid, admin, limit, q, cursor }) {
...
router.get('/ice', async (req, res) => {
  ...
  try {
    const get = async (k) => (await pg.get('SELECT value FROM app_settings WHERE key=?', k))?.value;
...
router.get('/groups', async (req, res) => {
  try {
    ...
    const groups = sortGroups(await enrichGroups(pg, uid, rows)).map(({ last_id, ...g }) => g);
...
router.delete('/:groupId/members/:userId', requirePermission('site_chat', 'create'), async (req, res) => {
  try {
    ...
    await pg.run('DELETE FROM chat_group_members WHERE group_id=? AND user_id=?', g, +req.params.userId);
    emitChat(g, 'changed', { groupId: g });
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
