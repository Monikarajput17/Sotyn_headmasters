...
async function generateCustomerCode() {
  ...
  return nextSequencePg(pg, 'customers', 'customer_code', 'CUST-', { startFrom: 1000, pad: 5 });
}
...
router.get('/', requirePermission('customers', 'view'), async (req, res) => {
  try {
    ...
    if (search) {
      sql += ' AND (company_name ILIKE ? OR sub_company_name ILIKE ? OR customer_code ILIKE ? OR contact_no ILIKE ? OR email ILIKE ? OR concern_person_name ILIKE ?)';
      const q = `%${search}%`;
...
router.get('/:id', requirePermission('customers', 'view'), async (req, res) => {
  try {
    const row = await pg.get('SELECT * FROM customers WHERE id=?', req.params.id);
...
router.post('/', requirePermission('customers', 'create'), async (req, res) => {
  try {
    ...
    const cnameDup = await findDuplicate(pg, {
    ...
    const r = await pg.run(
      ...
      code, b.category || '', b.company_name.trim(), b.sub_company_name || '', b.company_registration_address || '', b.contact_no || '', b.email || '', b.concern_person_name || '', b.concern_person_email || '', b.concern_person_address || '');
...
router.put('/:id', requirePermission('customers', 'edit'), async (req, res) => {
  try {
    ...
    await pg.run(
      ...
      b.category || '', b.company_name || '', b.sub_company_name || '', b.company_registration_address || '', b.contact_no || '', b.email || '', b.concern_person_name || '', b.concern_person_email || '', b.concern_person_address || '', req.params.id);
...
router.delete('/:id', requirePermission('customers', 'delete'), async (req, res) => {
  try {
    await pg.run('DELETE FROM customers WHERE id=?', req.params.id);
    res.json({ message: 'Deleted' });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// POST bulk-import (Excel upload)
router.post('/bulk-import', requirePermission('customers', 'create'), upload.single('file'), async (req, res) => {
