...
router.get('/', async (req, res) => {
  try {
    const isQuoter = await canQuote(req);
...
router.put('/:id', async (req, res) => {
  try {
    const cur = await pg.get('SELECT raised_by, status FROM price_requests WHERE id=?', req.params.id);
    ...
    const full = await pg.get('SELECT * FROM price_requests WHERE id=?', req.params.id);
    await pg.run(`UPDATE price_requests SET
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    await pg.run('DELETE FROM price_requests WHERE id=?', req.params.id);
...
router.get('/grouped', async (req, res) => {
  try {
    if (!(await canQuote(req))) return res.status(403).json({ error: 'Quoting permission required' });
...
router.put('/:id/rate', async (req, res) => {
  try {
    if (!(await canQuote(req))) return res.status(403).json({ error: 'Quoting permission required' });
    const cur = await pg.get('SELECT * FROM price_requests WHERE id=?', req.params.id);
    if (!cur) return res.status(404).json({ error: 'Not found' });
    ...
    await pg.run(`UPDATE price_requests SET ${sets.join(', ')} WHERE id=?`, ...params);
...
router.post('/:id/finalize', async (req, res) => {
  try {
    ...
    const allOpen = await pg.all(`SELECT * FROM price_requests WHERE status IN ('open','quoted')`);
    ...
    for (const s of siblings) {
      await pg.run(`
        UPDATE price_requests
         ...
         WHERE id = ?
      `, final_vendor_name, +final_rate, final_terms || null, req.user.id, newMasterId, s.id);
    }
...
router.post('/bulk-upload', bulkUpload.single('file'), async (req, res) => {
  ...
  try {
    await pg.tx(async (t) => {
      for (let idx = 0; idx < rows.length; idx++) {
        ...
        if (isSample) { skipped.push({ row: idx + 2, reason: 'Sample row skipped' }); continue; }
