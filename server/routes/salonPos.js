...
router.get('/settings', requirePermission(M, 'view'), async (req, res) => {
  try {
...
router.post('/', requirePermission(M, 'create'), async (req, res) => {
  try {
    ...
    let client = b.client_id ? await pg.get('SELECT * FROM salon_clients WHERE id=?', b.client_id) : null;
