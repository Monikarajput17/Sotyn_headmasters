...
// ── Schema (idempotent) ───────────────────────────────────────────
// Postgres migration: date/datetime columns are TEXT (same stored format as
// SQLite), ids are identity columns so RETURNING id keeps lastInsertRowid
// working. Runs async at module load; seeds only when tables are empty.
(async () => {
try {
  await pg.query(`
  ...
  const ruleCount = (await pg.get('SELECT COUNT(*) AS n FROM procurement_phase_rules')).n;
  if (ruleCount === 0) {
    for (const { category, vendor_lead } of DEFAULT_CATEGORIES) {
      for (const phase of PHASES) {
        const days = phase === 'dispatch' ? vendor_lead : FIXED_PHASE_DAYS[phase];
        await pg.run('INSERT INTO procurement_phase_rules (category, phase, days) VALUES (?, ?, ?)', category, phase, days);
      }
    }
  ...
  try {
    const done = await pg.get("SELECT value FROM app_settings WHERE key='procsch_indent_one_day_v1'");
    if (!done) {
      const r = await pg.run(`UPDATE procurement_phase_rules SET days=1 WHERE phase='indent' AND days <> 1`);
      await pg.run("INSERT INTO app_settings (key, value) VALUES ('procsch_indent_one_day_v1', '1')");
  ...
  const holCount = (await pg.get('SELECT COUNT(*) AS n FROM procurement_holidays')).n;
  if (holCount === 0) {
    for (const [d, label] of SEED_HOLIDAYS_2026) {
      await pg.run('INSERT INTO procurement_holidays (holiday_date, label) VALUES (?, ?) ON CONFLICT DO NOTHING', d, label);
    }
...
// (Legacy guarded `ALTER TABLE ... ADD COLUMN ai_reasoning` dropped — the
// migrated Postgres schema already carries the column.)
...
async function getAiSetting(key) {
  try { return (await pg.get('SELECT value FROM app_settings WHERE key = ?', key))?.value; } catch (_) { return null; }
}
...
async function loadHolidays() {
  const rows = await pg.all('SELECT holiday_date FROM procurement_holidays');
...
// ── Helpers ───────────────────────────────────────────────────────
async function getPhaseDaysMap() {
  const rows = await pg.all('SELECT category, phase, days FROM procurement_phase_rules');
  const map = {};
  for (const r of rows) {
...
router.get('/phase-rules', requirePermission('procurement_schedule', 'view'), async (req, res) => {
  try {
    const rows = await pg.all('SELECT category, phase, days FROM procurement_phase_rules ORDER BY category, phase');
...
router.put('/phase-rules', requirePermission('procurement_schedule', 'edit'), async (req, res) => {
  try {
...
router.get('/holidays', requirePermission('procurement_schedule', 'view'), async (req, res) => {
  try {
    res.json(await pg.all('SELECT id, holiday_date, label FROM procurement_holidays ORDER BY holiday_date'));
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// POST /procurement-schedule/holidays — add one
router.post('/holidays', requirePermission('procurement_schedule', 'edit'), async (req, res) => {
  const { holiday_date, label } = req.body || {};
  if (!holiday_date) return res.status(400).json({ error: 'holiday_date required (YYYY-MM-DD)' });
  try {
    const r = await pg.run('INSERT INTO procurement_holidays (holiday_date, label) VALUES (?, ?)', holiday_date, label || null);
...
router.delete('/holidays/:id', requirePermission('procurement_schedule', 'edit'), async (req, res) => {
  try {
    await pg.run('DELETE FROM procurement_holidays WHERE id = ?', req.params.id);
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.get('/:project_id/meta', requirePermission('procurement_schedule', 'view'), async (req, res) => {
  try {
    ...
    const project = await pg.get(
      `SELECT bb.id, bb.company_name AS project_name, bb.client_name,
         ...
         FROM business_book bb WHERE bb.id = ?`, pid);
    if (!project) return res.status(404).json({ error: 'Project not found' });
    const meta = await pg.get('SELECT start_date, end_date, client_requirements, updated_at FROM procurement_schedule_meta WHERE project_id = ?', pid);
    const drawings = await pg.all(
      `SELECT d.id, d.filename, d.file_size, d.uploaded_at, u.name AS uploaded_by_name
         FROM procurement_schedule_drawings d
         LEFT JOIN users u ON u.id = d.uploaded_by
        WHERE d.project_id = ?
        ORDER BY d.uploaded_at DESC`, pid);
    res.json({
      project,
...
router.put('/:project_id/meta', requirePermission('procurement_schedule', 'edit'), async (req, res) => {
  try {
    const pid = +req.params.project_id;
    const { start_date, end_date, client_requirements } = req.body || {};
    const project = await pg.get('SELECT id FROM business_book WHERE id = ?', pid);
    if (!project) return res.status(404).json({ error: 'Project not found' });
    await pg.run(
      ...
      pid, start_date || null, end_date || null, client_requirements ?? null, req.user.id);
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.post('/:project_id/drawings', requirePermission('procurement_schedule', 'edit'),
  drawingUpload.single('file'), async (req, res) => {
    try {
      const pid = +req.params.project_id;
      ...
      const r = await pg.run(
        `INSERT INTO procurement_schedule_drawings
         ...
         VALUES (?, ?, ?, ?, ?, ?)`,
        pid, req.file.originalname, req.file.filename, req.file.mimetype, req.file.size, req.user.id);
...
router.get('/drawing/:fileId', requirePermission('procurement_schedule', 'view'), async (req, res) => {
  try {
    const f = await pg.get('SELECT filename, storage_path, file_type FROM procurement_schedule_drawings WHERE id = ?', +req.params.fileId);
...
router.delete('/drawing/:fileId', requirePermission('procurement_schedule', 'edit'), async (req, res) => {
  try {
    const f = await pg.get('SELECT storage_path FROM procurement_schedule_drawings WHERE id = ?', +req.params.fileId);
    if (!f) return res.status(404).json({ error: 'File not found' });
    try { fs.unlinkSync(path.join(drawingDir, f.storage_path)); } catch (_) {}
    await pg.run('DELETE FROM procurement_schedule_drawings WHERE id = ?', +req.params.fileId);
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.get('/projects', requirePermission('procurement_schedule', 'view'), async (req, res) => {
  try {
...
router.get('/:project_id', requirePermission('procurement_schedule', 'view'), async (req, res) => {
  try {
    const pid = +req.params.project_id;
    const project = await pg.get('SELECT id, company_name AS project_name, client_name, committed_completion_date AS completion_date FROM business_book WHERE id = ?', pid);
    if (!project) return res.status(404).json({ error: 'Project not found' });
    const rows = await pg.all(`
...
router.post('/:project_id/ai-suggest', requirePermission('procurement_schedule', 'edit'), async (req, res) => {
  try {
  const apiKey = await getAiSetting('ai_api_key');
  if (!apiKey) {
  ...
  const project = await pg.get(
    `SELECT id, company_name AS project_name, client_name, committed_completion_date AS completion_date
       FROM business_book WHERE id = ?`, pid);
  if (!project) return res.status(404).json({ error: 'Project not found' });
  ...
  const meta = await pg.get('SELECT start_date, end_date, client_requirements FROM procurement_schedule_meta WHERE project_id = ?', pid);
  const bbDates = await pg.get('SELECT committed_start_date, committed_completion_date FROM business_book WHERE id = ?', pid);
  ...
  const drawings = await pg.all(
    'SELECT id, filename, storage_path, file_type, file_size FROM procurement_schedule_drawings WHERE project_id = ? ORDER BY uploaded_at', pid);
...
[{"item_id": <number>, "trade": "<string>", "dispatch_days": <number>, "reasoning": "<string>"}, ...]`;
  ...
  try {
    ...
    const model = (await getAiSetting('ai_model')) || 'claude-opus-4-7';
...
router.post('/:project_id/regenerate', requirePermission('procurement_schedule', 'edit'), async (req, res) => {
  try {
  const pid = +req.params.project_id;
  const project = await pg.get('SELECT id, company_name AS project_name, committed_completion_date AS completion_date FROM business_book WHERE id = ?', pid);
  if (!project) return res.status(404).json({ error: 'Project not found' });
  ...
  const meta = await pg.get('SELECT end_date FROM procurement_schedule_meta WHERE project_id = ?', pid);
  ...
  const rules = await getPhaseDaysMap();
  ...
  await pg.tx(async (t) => {
    await t.run('DELETE FROM procurement_schedule WHERE project_id = ?', pid);
    for (const r of newRows) {
      await t.run(`INSERT INTO procurement_schedule
        (project_id, item_id, trade, phase, start_date, end_date, status, lead_days, ai_reasoning)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        r.project_id, r.item_id, r.trade, r.phase, r.start_date, r.end_date, r.status, r.lead_days, r.ai_reasoning);
    }
  });

  const earliestIndent = (await pg.get(
    `SELECT MIN(start_date) AS d FROM procurement_schedule WHERE project_id = ? AND phase = 'indent'`, pid)).d;
  ...
  try {
    const metaForSnap = await pg.get('SELECT start_date, end_date, client_requirements FROM procurement_schedule_meta WHERE project_id = ?', pid);
    const label = `${overrides.size > 0 ? 'AI · ' : ''}Approved by ${req.user?.name || 'user'} · ${new Date().toISOString().slice(0,16).replace('T',' ')}`;
    await pg.run(`INSERT INTO procurement_schedule_snapshots
...
router.get('/:project_id/snapshots', requirePermission('procurement_schedule', 'view'), async (req, res) => {
  try {
    const rows = await pg.all(`
...
router.get('/snapshot/:id', requirePermission('procurement_schedule', 'view'), async (req, res) => {
  try {
  const snap = await pg.get(`
...
router.delete('/snapshot/:id', requirePermission('procurement_schedule', 'delete'), async (req, res) => {
  try {
    await pg.run('DELETE FROM procurement_schedule_snapshots WHERE id = ?', +req.params.id);
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
