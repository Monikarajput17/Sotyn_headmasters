...
const stUserEmail = async (id) => { try { return (await pg.get('SELECT email FROM users WHERE id=?', id))?.email || null; } catch { return null; } };
const stDirector = () => { try { return require('../lib/email').getDirectorEmail(); } catch { return null; } };
  ...
  try {
    ...
    try {
      ...
      if (assigned_to) {
        ...
        const admins = (await pg.all(`SELECT id FROM users WHERE role='admin' AND COALESCE(active,1)=1`)).map(u => u.id);
        notifyMany(admins, {
