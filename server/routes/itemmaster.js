...
// Whole days since that rate date (all date columns are text; UTC today).
const AGE_DAYS_EXPR = `((now() at time zone 'utc')::date - LEFT(${AGE_DATE_EXPR},10)::date)`;
...
(async () => {
  for (const sql of [
    `CREATE INDEX IF NOT EXISTS idx_im_department    ON item_master(department)`,
    `CREATE INDEX IF NOT EXISTS idx_im_vendor_id     ON item_master(vendor_id)`,
    `CREATE INDEX IF NOT EXISTS idx_im_make          ON item_master(make)`,
    `CREATE INDEX IF NOT EXISTS idx_im_bill_po_date  ON item_master(bill_po_date)`,
    `CREATE INDEX IF NOT EXISTS idx_im_item_code     ON item_master(item_code)`,
    `CREATE INDEX IF NOT EXISTS idx_im_approval       ON item_master(approval_status)`,
  ]) {
    try { await pg.run(sql); } catch (_) {}
  }
})();

// NOTE: the old guarded `ALTER TABLE item_master ADD COLUMN ...` block
// (priced_at / priced_by / approved_by / approved_at / approval_status —
// mam 2026-06-16 "data is missing") is gone: the Postgres migration
// guarantees those columns exist, so the SQLite-build workaround is moot.
...
(async () => {
  try {
    ...
    const flag = await pg.get(`SELECT value FROM app_settings WHERE key='item_master_recent_pending_v1'`);
    if (!flag) {
      const cutoff = new Date(Date.now() - 2 * 86400000).toISOString().slice(0, 19).replace('T', ' ');
      const r = await pg.run(`
        UPDATE item_master
         ...
         WHERE COALESCE(approval_status, 'approved') = 'approved'
           AND created_at >= ?
      `, cutoff);
      await pg.run(`INSERT INTO app_settings (key, value) VALUES ('item_master_recent_pending_v1', '1')`);
...
function buildItemFilters(query) {
  ...
  if (search) {
    clauses.push('(im.item_name ILIKE ? OR im.specification ILIKE ? OR im.size ILIKE ? OR im.item_code ILIKE ? OR im.make ILIKE ?)');
    const q = `%${search}%`;
    params.push(q, q, q, q, q);
  }
  if (status === 'expired')    clauses.push(`${AGE_DAYS_EXPR} > 60`);
  if (status === 'ageing')     clauses.push(`${AGE_DAYS_EXPR} BETWEEN 31 AND 60`);
  if (status === 'fresh')      clauses.push(`${AGE_DAYS_EXPR} <= 30 AND ${AGE_DATE_EXPR} IS NOT NULL`);
...
router.get('/', requirePermission('item_master', 'view'), async (req, res) => {
  ...
  try {
    const rows  = await pg.all(sql, ...params, limit, offset);
    const total = (await pg.get(countSql, ...params)).n;
    res.json({
    ...
    try {
      const fbSql = `
        ...
        ORDER BY im.item_code
        LIMIT ? OFFSET ?`;
      const rows  = await pg.all(fbSql, ...params, limit, offset);
      const total = (await pg.get(countSql, ...params)).n;
      res.json({
        items: rows.map(r => ({ ...r, age_status: ageStatus(r.age_days) })),
        total, limit, offset,
...
router.get('/completion', requirePermission('item_master', 'view'), async (req, res) => {
  try {
...
router.get('/dropdown', async (req, res) => {
  try {
    ...
    // Postgres note: SQLite's "single MIN(id) aggregate + bare columns" trick
    // (which made every column come from the min-id row, mam 2026-06-10) is
    // expressed here as DISTINCT ON (...) ORDER BY ..., id — same semantics:
    // one row per unique key, all columns from the LOWEST-id row.
    ...
    // else priced_at, else updated_at when a price exists) — and the date comes
    // from the same representative (lowest-id) row.
    const ageExpr = `COALESCE(bill_po_date, priced_at, CASE WHEN current_price > 0 THEN updated_at END)`;
    const sql = `SELECT * FROM (
                   SELECT DISTINCT ON (LOWER(TRIM(item_name)), LOWER(TRIM(COALESCE(specification, ''))), LOWER(TRIM(COALESCE(size, ''))))
                          id, item_code, department, item_name, specification, size, uom, gst, type, make, current_price,
                          COALESCE(approval_status, 'approved') AS approval_status,
                          CASE WHEN ${ageExpr} IS NULL THEN NULL
                               ELSE ((now() at time zone 'utc')::date - LEFT(${ageExpr},10)::date) END AS age_days
                     FROM item_master ${where}
                    ORDER BY LOWER(TRIM(item_name)), LOWER(TRIM(COALESCE(specification, ''))), LOWER(TRIM(COALESCE(size, ''))), id
                 ) dedup
                 ORDER BY department, item_name`;
    const items = types.length ? await pg.all(sql, ...types) : await pg.all(sql);
...
router.get('/:id', requirePermission('item_master', 'view'), async (req, res) => {
  try {
...
router.get('/:id/price-history', requirePermission('item_master', 'view'), async (req, res) => {
  try {
...
async function snapshotCurrentPrice(db, oldRow, actor) {
  if (!oldRow) return;
...
router.patch('/:id/identity', requirePermission('item_master', 'edit'), async (req, res) => {
  try {
    const b = req.body || {};
    const cur = await pg.get('SELECT item_name, specification, make FROM item_master WHERE id=?', req.params.id);
    ...
    await pg.run('UPDATE item_master SET item_name=?, specification=?, make=?, updated_at=CURRENT_TIMESTAMP WHERE id=?',
      item_name, specification, make, req.params.id);
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// Inline price patch (Inventory page uses this). Snapshots history.
router.patch('/:id/price', requirePermission('item_master', 'edit'), async (req, res) => {
  try {
    const old = await pg.get('SELECT * FROM item_master WHERE id=?', req.params.id);
    if (!old) return res.status(404).json({ error: 'Not found' });
    const price = +req.body?.current_price;
    if (!(price >= 0)) return res.status(400).json({ error: 'Price must be a positive number' });
    await snapshotCurrentPrice(pg, old, req.user);
    await pg.run(`
      UPDATE item_master
         SET current_price = ?,
       ...
       WHERE id = ?
    `, price, req.body?.source_type || null, req.user.id, req.params.id);
...
router.get('/approval/pending-count', async (req, res) => {
  try {
    const n = (await pg.get(`SELECT COUNT(*) AS n FROM item_master WHERE approval_status='pending'`)).n;
    res.json({ pending: n });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// Approve everything currently pending — convenience for clearing a backlog.
router.post('/approval/approve-all', adminOnly, async (req, res) => {
  try {
    const r = await pg.run(`UPDATE item_master
         SET approval_status='approved', approved_by=?, approved_at=CURRENT_TIMESTAMP, updated_at=CURRENT_TIMESTAMP
       WHERE approval_status='pending'`, req.user.id);
...
router.post('/:id/approve', adminOnly, async (req, res) => {
  try {
    const item = await pg.get('SELECT id FROM item_master WHERE id=?', req.params.id);
    if (!item) return res.status(404).json({ error: 'Not found' });
    await pg.run(`UPDATE item_master
                   SET approval_status='approved', approved_by=?, approved_at=CURRENT_TIMESTAMP, updated_at=CURRENT_TIMESTAMP
...
router.post('/', requirePermission('item_master', 'create'), async (req, res) => {
  try {
    ...
    if (!code) {
      ...
      code = await nextSequencePg(pg, 'item_master', 'item_code', dept, { startFrom: 0, pad: 4 });
    }
...
router.put('/:id', requirePermission('item_master', 'edit'), async (req, res) => {
  try {
    ...
    if (priceProvenanceChanged) await snapshotCurrentPrice(pg, old, req.user);
    await pg.run(`
      UPDATE item_master
         SET item_code = ?, department = ?, item_name = ?, specification = ?, size = ?,
             ...
             vendor_id = ?, source_type = ?, bill_po_number = ?, bill_po_date = ?,
             weight_per_meter = ?, weight_per_pipe = ?, pipe_length_m = ?,
             priced_at = CASE WHEN ?=1 THEN CURRENT_TIMESTAMP ELSE priced_at END,
             priced_by = CASE WHEN ?=1 THEN ? ELSE priced_by END,
             updated_at = CURRENT_TIMESTAMP
       WHERE id = ?
    `,
...
router.delete('/:id', requirePermission('item_master', 'delete'), async (req, res) => {
  try {
    await pg.run('DELETE FROM item_master WHERE id=?', req.params.id);
...
// still work.
router.post('/bulk', requirePermission('item_master', 'create'), async (req, res) => {
  try {
    ...
    const vendorByName = new Map(
      (await pg.all('SELECT id, LOWER(TRIM(name)) AS k FROM vendors')).map(r => [r.k, r.id])
    );
    const insertSql = `
      INSERT INTO item_master
