...
async function nextBillNumber(db, dateStr) {
  const prefix = `SEPL/SB/${fyLabel(dateStr)}/`;
  let max = 0;
  for (const r of await db.all('SELECT bill_number FROM sales_bills WHERE bill_number LIKE ?', prefix + '%')) {
...
router.get('/orders', requirePermission('installation', 'view'), async (req, res) => {
  try {
...
router.get('/orders/:bbId', requirePermission('installation', 'view'), async (req, res) => {
  try {
    const bb = await pg.get('SELECT * FROM business_book WHERE id=?', req.params.bbId);
...
router.get('/', requirePermission('installation', 'view'), async (req, res) => {
  try {
    const rows = await pg.all(
...
router.get('/pending', requirePermission('installation', 'view'), async (req, res) => {
  try {
    const ordersWithoutSo = (await pg.all(
...
router.get('/material', requirePermission('installation', 'view'), async (req, res) => {
  try {
    let rows = [];
    try {
      rows = await pg.all(
    ...
    const resolveBb = async (indentId, siteName) => {
      const key = indentId ? ('i' + indentId) : ('s:' + (siteName || ''));
      if (indentBb.has(key)) return indentBb.get(key);
      let bbId = null;
      try {
        if (indentId) {
          const r = await pg.get('SELECT op.business_book_id AS bb FROM indents i LEFT JOIN order_planning op ON op.id = i.planning_id WHERE i.id=?', indentId);
          bbId = (r && r.bb) || null;
          if (!bbId) { const i2 = await pg.get('SELECT site_name FROM indents WHERE id=?', indentId); siteName = (i2 && i2.site_name) || siteName; }
        }
        if (!bbId && siteName) {
          const s = await pg.get('SELECT business_book_id AS bb FROM sites WHERE LOWER(TRIM(name))=LOWER(TRIM(?)) AND business_book_id IS NOT NULL LIMIT 1', siteName);
          bbId = (s && s.bb) || null;
        }
      } catch (_) {}
    ...
    const indentItemsSql = 'SELECT quantity, po_item_id, description, rate FROM indent_items WHERE indent_id=?';

    const out = [];
    for (const r of rows) {
      const bb = await getBb(await resolveBb(r.indent_id, r.site_name));
      const pct = bb ? bb.pct : 0;
      let boqValue = 0, itemCount = 0;
      if (r.indent_id) {
        try {
          const items = await pg.all(indentItemsSql, r.indent_id);
...
// One bill + its items.
router.get('/:id', requirePermission('installation', 'view'), async (req, res) => {
  try {
    const bill = await pg.get('SELECT * FROM sales_bills WHERE id=? AND bill_type IS NOT NULL', req.params.id);
    if (!bill) return res.status(404).json({ error: 'Bill not found' });
    bill.items = await pg.all('SELECT * FROM sales_bill_items WHERE sales_bill_id=? ORDER BY id', bill.id);
    bill.log = await pg.all(
...
router.get('/:id/print', requirePermission('installation', 'view'), async (req, res) => {
  try {
    const bill = await pg.get('SELECT * FROM sales_bills WHERE id=? AND bill_type IS NOT NULL', req.params.id);
    if (!bill) return res.status(404).send('Bill not found');
    let items = await pg.all('SELECT * FROM sales_bill_items WHERE sales_bill_id=? ORDER BY id', bill.id);
      ...
      try {
        wi = await pg.all(
          `SELECT wi.description, wi.unit, wi.rate,
            ...
            GROUP BY COALESCE(wi.po_item_id::text, wi.description), wi.description, wi.unit, wi.rate
            ORDER BY MIN(wi.id)`,
          bill.id);
      } catch (_) { wi = []; }
    ...
    if (bill.business_book_id && items.some(it => !(+it.rate > 0))) {
      ...
      for (const p of await pg.all('SELECT description, rate FROM po_items WHERE business_book_id=?', bill.business_book_id)) {
    ...
    const bb = bill.business_book_id ? await pg.get('SELECT * FROM business_book WHERE id=?', bill.business_book_id) : null;
...
router.post('/', requirePermission('installation', 'create'), async (req, res) => {
  try {
    ...
    const bb = await pg.get('SELECT * FROM business_book WHERE id=?', business_book_id);
    if (!bb) return res.status(404).json({ error: 'Order not found' });
    ...
    const out = await pg.tx(async (t) => {
      const bill_number = await nextBillNumber(t, bill_date);
      const r = await t.run(
      ...
      for (const it of items) {
        ...
        await t.run(
          `INSERT INTO sales_bill_items (sales_bill_id, description, qty_ordered, qty_delivered, unit, rate, amount)
           VALUES (?,?,?,?,?,?,?)`,
          billId, it.description || '', round2(it.qty_ordered), round2(it.qty_delivered), it.unit || '', round2(it.rate), round2(it.amount));
      }
      await t.run('INSERT INTO sales_bill_status_log (sales_bill_id, status, changed_by, notes) VALUES (?,?,?,?)',
        billId, 'draft', req.user.id, `${REF_TYPE[bill_type]} bill created`);
...
router.put('/:id/approve', requirePermission('installation', 'edit'), async (req, res) => {
  try {
    const bill = await pg.get('SELECT id, approval_status FROM sales_bills WHERE id=? AND bill_type IS NOT NULL', req.params.id);
    if (!bill) return res.status(404).json({ error: 'Bill not found' });
    const next = req.body.approval_status === 'draft' ? 'draft' : 'approved';
    await pg.run('UPDATE sales_bills SET approval_status=? WHERE id=?', next, bill.id);
    await pg.run('INSERT INTO sales_bill_status_log (sales_bill_id, status, changed_by, notes) VALUES (?,?,?,?)',
      bill.id, next, req.user.id, next === 'approved' ? 'Approved' : 'Reverted to draft');
...
router.delete('/:id', requirePermission('installation', 'delete'), async (req, res) => {
  try {
    const bill = await pg.get('SELECT id FROM sales_bills WHERE id=? AND bill_type IS NOT NULL', req.params.id);
    if (!bill) return res.status(404).json({ error: 'Bill not found' });
    const child = await pg.get('SELECT id FROM sales_bills WHERE previous_bill_id=?', bill.id);
    ...
    await pg.run('UPDATE dpr SET sales_bill_id=NULL WHERE sales_bill_id=?', bill.id);
    await pg.run('DELETE FROM sales_bills WHERE id=?', bill.id);
...
router.put('/:id/sent', requirePermission('installation', 'edit'), async (req, res) => {
  try {
    const bill = await pg.get('SELECT id, sent_to_client FROM sales_bills WHERE id=? AND bill_type IS NOT NULL', req.params.id);
    if (!bill) return res.status(404).json({ error: 'Bill not found' });
    const sent = bill.sent_to_client ? 0 : 1;
    await pg.run('UPDATE sales_bills SET sent_to_client=?, sent_at=' + (sent ? 'CURRENT_TIMESTAMP' : 'NULL') + ' WHERE id=?', sent, bill.id);
    await pg.run('INSERT INTO sales_bill_status_log (sales_bill_id, status, changed_by, notes) VALUES (?,?,?,?)',
      bill.id, sent ? 'sent_to_client' : 'unsent', req.user.id, sent ? 'Sent to client' : 'Marked not sent');
...
router.post('/:id/payment', requirePermission('installation', 'edit'), async (req, res) => {
  try {
    const bill = await pg.get('SELECT * FROM sales_bills WHERE id=? AND bill_type IS NOT NULL', req.params.id);
    if (!bill) return res.status(404).json({ error: 'Bill not found' });
    ...
    const out = await pg.tx(async (t) => {
      await t.run(
        ...
        bill.id, amount, payment_date, payment_mode, req.body.transaction_ref || null, req.body.notes || null, req.user.id);

      const received = round2((await t.get(
      ...
      await t.run('UPDATE sales_bills SET payment_status=? WHERE id=?', pstatus, bill.id);
      await t.run('INSERT INTO sales_bill_status_log (sales_bill_id, status, changed_by, notes) VALUES (?,?,?,?)',
        bill.id, pstatus, req.user.id, `Payment ₹${amount} (${payment_mode})`);
      ...
      const existing = await t.get('SELECT id FROM receivables WHERE invoice_number=?', bill.bill_number);
      if (existing) {
        await t.run('UPDATE receivables SET received_amount=?, outstanding_amount=?, status=?, updated_at=CURRENT_TIMESTAMP WHERE id=?',
          received, outstanding, rstatus, existing.id);
      } else {
        await t.run(
          `INSERT INTO receivables (client_name, project_name, business_book_id, invoice_number, invoice_date,
           ...
           VALUES (?,?,?,?,?,?,?,?,?,?)`,
          bill.customer_name || 'Customer', bill.project_name || null, bill.business_book_id, bill.bill_number,
...
// `db` is the pg adapter module (needs .all/.get/.run/.tx).
async function generateInstallationBills(db, userId, { draft = true } = {}) {
  ...
  await db.tx(async (t) => {
    for (const [bbId, g] of groups) {
      ...
      const bb = await t.get('SELECT * FROM business_book WHERE id=?', bbId);
      if (!bb) continue;
      ...
      const gst_amount = round2(amount * gst_rate / 100);
      const total_amount = round2(amount + gst_amount);
      const bill_number = await nextBillNumber(t, today);
      const r = await t.run(
        `INSERT INTO sales_bills
           (bill_number, bill_date, amount, gst_amount, total_amount, gst_rate,
            bill_type, business_book_id, customer_name, project_name, bill_status,
        ...
        bill_number, today, amount, gst_amount, total_amount, gst_rate,
      ...
      const billId = r.lastInsertRowid;
      for (const dprId of g.dprIds) await t.run('UPDATE dpr SET sales_bill_id=? WHERE id=?', billId, dprId);
      await t.run('INSERT INTO sales_bill_status_log (sales_bill_id, status, changed_by, notes) VALUES (?,?,?,?)',
        billId, draft ? 'draft' : 'approved', userId, `Auto installation bill from ${g.dprIds.length} DPR(s)`);
...
router.post('/generate-installation', requirePermission('installation', 'create'), async (req, res) => {
  try {
    const result = await generateInstallationBills(pg, req.user.id, { draft: false });
