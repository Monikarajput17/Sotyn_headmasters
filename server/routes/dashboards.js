...
router.get('/pending-approvals', async (req, res) => {
  try {
    ...
    const safe = async (sql, ...a) => { try { return (await pg.get(sql, ...a))?.c || 0; } catch (_) { return 0; } };
    const items = [
      { key: 'indents', label: 'Indent Approval', icon: '📋',
        count: await safe("SELECT COUNT(*) c FROM indents WHERE status IN ('submitted','l1_approved','crm_approved')"),
        link: '/procurement?tab=indents' },
      { key: 'vendor_po', label: 'Vendor PO Approval', icon: '🧾',
        count: await safe("SELECT COUNT(*) c FROM vendor_pos WHERE po_approval IN ('pending_l1','pending_l2')"),
      ...
      { key: 'payment', label: 'Payment Approval', icon: '💸',
        ...
        count: await safe("SELECT COUNT(*) c FROM payment_requests WHERE current_step=3 AND status NOT IN ('final_approved','rejected')"),
        link: '/payment-required' },
      { key: 'dpr', label: 'DPR Approval', icon: '📝',
        count: await safe("SELECT COUNT(*) c FROM dpr WHERE approval_status='pending'"),
        link: '/dpr' },
      { key: 'delegation', label: 'Delegation Sign-off', icon: '✅',
        count: await safe("SELECT COUNT(*) c FROM delegations WHERE assigned_by=? AND status='submitted'", uid),
        link: '/delegations' },
    ];
...
router.get('/pending-approvals/:key', async (req, res) => {
  try {
    const uid = req.user.id;
    const all = async (sql, ...a) => { try { return await pg.all(sql, ...a); } catch (_) { return []; } };
    let items = [];
    switch (req.params.key) {
      case 'indents':
        items = (await all(`SELECT id, indent_number AS title, COALESCE(NULLIF(site_name,''),'—') AS subtitle, status AS meta
      ...
      case 'vendor_po':
        ...
        items = (await all(`SELECT vp.id, vp.po_number AS title, COALESCE(v.name,'—') AS subtitle, vp.po_approval AS meta, vp.total_amount AS amount
      ...
      case 'payment':
        ...
        items = (await all(`SELECT id, COALESCE(NULLIF(vendor_name,''), employee_name, '—') AS title,
        ...
        break;
      case 'dpr':
        items = (await all(`SELECT d.id, COALESCE(s.name,'Site #'||d.site_id) AS title, d.report_date AS subtitle, 'pending' AS meta
        ...
        break;
      case 'delegation':
        items = (await all(`SELECT d.id, COALESCE(d.title,'Task') AS title, COALESCE(u.name,'') AS subtitle, 'submitted' AS meta
...
router.post('/approve/:key/:id', adminOnly, async (req, res) => {
  const uid = req.user.id;
  const id = +req.params.id;
  try {
    if (req.params.key === 'indents') {
      await pg.run(`UPDATE indents SET
        ...
        WHERE id=?`, uid, uid, uid, id);
    } else if (req.params.key === 'vendor_po') {
      await pg.run(`UPDATE vendor_pos SET po_approval='approved',
          po_l1_by=COALESCE(po_l1_by,?), po_l1_at=COALESCE(po_l1_at,CURRENT_TIMESTAMP),
          po_l2_by=?, po_l2_at=CURRENT_TIMESTAMP WHERE id=?`, uid, uid, id);
    } else {
...
router.get('/cmd-detail', adminOnly, async (req, res) => {
  try {
    res.json(await computeCmdDetail(pg, req.query.days));
  } catch (e) {
