...
// ── Helpers ────────────────────────────────────────────────────
async function nextEnquiryNo(db) {
  const yr = new Date().getFullYear();
  const last = await db.get(`SELECT enquiry_no FROM rental_tool_enquiry WHERE enquiry_no LIKE ? ORDER BY id DESC LIMIT 1`, `RT-${yr}-%`);
  let n = 1;
  if (last?.enquiry_no) {
...
async function getApproverId(db) {
  const row = await db.get(`SELECT value FROM app_settings WHERE key='rental_approver_user_id'`);
...
async function getStageLabels(db) {
  try {
    const row = await db.get(`SELECT value FROM app_settings WHERE key='rental_tools_stage_labels'`);
...
async function canApprove(db, userId) {
  const approverId = await getApproverId(db);
...
router.get('/dashboard', requirePermission('rental_tools', 'view'), async (req, res) => {
  try {
    ...
    const counts = {
      enquiry:           (await pg.get(`SELECT COUNT(*) c FROM rental_tool_enquiry WHERE current_stage='enquiry' AND status='open'`)).c,
      rate_finalised:    (await pg.get(`SELECT COUNT(*) c FROM rental_tool_enquiry WHERE current_stage='rate_finalised' AND status='open'`)).c,
      material_received: (await pg.get(`SELECT COUNT(*) c FROM rental_tool_enquiry WHERE current_stage='material_received' AND status='open'`)).c,
      returned:          (await pg.get(`SELECT COUNT(*) c FROM rental_tool_enquiry WHERE current_stage='returned'`)).c,
      cancelled:         (await pg.get(`SELECT COUNT(*) c FROM rental_tool_enquiry WHERE status='cancelled'`)).c,
      all:               (await pg.get(`SELECT COUNT(*) c FROM rental_tool_enquiry`)).c,
    };
    const now = new Date().toISOString();
    const today = now.slice(0, 10);   // UTC, exact parity with SQLite DATE('now')
    const breaches = {
      stage1_overdue: (await pg.get(`SELECT COUNT(*) c FROM rental_tool_enquiry WHERE current_stage='enquiry' AND stage1_target_at < ?`, now)).c,
      stage2_overdue: (await pg.get(`SELECT COUNT(*) c FROM rental_tool_enquiry WHERE current_stage='rate_finalised' AND stage2_target_at < ?`, now)).c,
      stage3_overdue: (await pg.get(`SELECT COUNT(*) c FROM rental_tool_enquiry WHERE current_stage='material_received' AND LEFT(return_target_date,10) < ?`, today)).c,
    };
    const open_total = counts.enquiry + counts.rate_finalised + counts.material_received;
    const total_value = (await pg.get(`SELECT COALESCE(SUM(vendor_rate * days_required),0) v FROM rental_tool_enquiry WHERE status='open' AND vendor_rate IS NOT NULL`)).v;
    // This-month enquiry count
    const thisMonth = (await pg.get(`SELECT COUNT(*) c FROM rental_tool_enquiry WHERE LEFT(created_at,7) = ?`, now.slice(0, 7))).c;
    res.json({
      counts, breaches, open_total, total_value, this_month: thisMonth,
      approver_user_id: await getApproverId(pg),
      stage_labels: await getStageLabels(pg),
    });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// ── GET / PUT /api/rental-tools/settings/stage-labels ──────────
router.get('/settings/stage-labels', requirePermission('rental_tools', 'view'), async (req, res) => {
  try {
    res.json({
      defaults: DEFAULT_STAGE_LABELS,
      current: await getStageLabels(pg),
    });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

router.put('/settings/stage-labels', requirePermission('rental_tools', 'edit'), async (req, res) => {
  try {
    ...
    await pg.run(`INSERT INTO app_settings (key, value) VALUES (?, ?) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value`,
      'rental_tools_stage_labels', JSON.stringify(cleaned));
    logAuditEvent({
    ...
    res.json({ current: await getStageLabels(pg) });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// ── GET /api/rental-tools/enquiries ────────────────────────────
router.get('/enquiries', requirePermission('rental_tools', 'view'), async (req, res) => {
  try {
    ...
    if (q) {
      sql += ' AND (e.site_name ILIKE ? OR e.enquiry_no ILIKE ? OR e.tool_description ILIKE ? OR e.vendor_name ILIKE ?)';
      const like = `%${q}%`;
...
router.get('/enquiries/:id', requirePermission('rental_tools', 'view'), async (req, res) => {
  try {
    const id = +req.params.id;
    const enquiry = await pg.get(`
      SELECT e.*, u.name as created_by_name,
             ap.name as rate_finalised_by_name,
             rb.name as return_signed_by_name,
    ...
    let po = null;
    if (enquiry.po_id) {
      po = await pg.get('SELECT * FROM purchase_orders WHERE id = ?', enquiry.po_id) || null;
    }
...
router.post('/enquiries', requirePermission('rental_tools', 'create'), async (req, res) => {
  const b = req.body || {};
  ...
  let id, enquiry_no;
  try {
    enquiry_no = await nextEnquiryNo(pg);
    ...
    await pg.run(`INSERT INTO rental_tool_history (enquiry_id, from_stage, to_stage, triggered_by, notes) VALUES (?, NULL, 'enquiry', ?, ?)`,
      id, String(req.user.id), `enquiry raised · stage 1 target ${stage1Target.toLocaleString('en-IN')}`);
  } catch (e) {
...
router.post('/enquiries/:id/finalise-rate', requirePermission('rental_tools', 'view'), async (req, res) => {
  try {
    const id = +req.params.id;
    if (!(await canApprove(pg, req.user.id))) {
    ...
    const enquiry = await pg.get(`SELECT * FROM rental_tool_enquiry WHERE id=?`, id);
    if (!enquiry) return res.status(404).json({ error: 'Enquiry not found' });
    ...
    try {
      poId = await pg.tx(async (t) => {
        ...
        const newPoId = poRes.lastInsertRowid;
        ...
        await t.run(`INSERT INTO rental_tool_history (enquiry_id, from_stage, to_stage, triggered_by, notes) VALUES (?, 'enquiry', 'rate_finalised', ?, ?)`,
          id, String(req.user.id), `vendor=${b.vendor_name} · rate=₹${b.vendor_rate}/${b.vendor_rate_unit || 'per_day'} · PO ${b.po_number} (₹${b.total_amount}) · stage 2 target ${stage2Target.toLocaleString('en-IN')}`);
        return newPoId;
      });
    ...
    try {
      await pg.run(`
        UPDATE rental_tool_enquiry SET
          ...
          updated_at = CURRENT_TIMESTAMP
        WHERE id = ?
      `, photoUrl, lat, lng, returnTarget.toISOString().slice(0, 10), id);
      await pg.run(`INSERT INTO rental_tool_history (enquiry_id, from_stage, to_stage, triggered_by, notes) VALUES (?, 'rate_finalised', 'material_received', ?, ?)`,
        id, String(req.user.id), `material received · photo uploaded · GPS ${lat.toFixed(5)},${lng.toFixed(5)} · return target ${returnTarget.toISOString().slice(0, 10)}`);
    } catch (e) {
      return res.status(500).json({ error: e.message });
    }
    logAuditEvent({
...
router.post('/enquiries/:id/return', requirePermission('rental_tools', 'view'), async (req, res) => {
  try {
    const id = +req.params.id;
    if (!(await canApprove(pg, req.user.id))) {
    ...
    try {
      await pg.run(`
        UPDATE rental_tool_enquiry SET
          ...
          status = 'closed',
          stage3_breached = CASE WHEN LEFT(return_target_date,10) < ? THEN 1 ELSE 0 END,
          updated_at = CURRENT_TIMESTAMP
        WHERE id = ?
      `, req.user.id, notes || null, today, id);
      await pg.run(`INSERT INTO rental_tool_history (enquiry_id, from_stage, to_stage, triggered_by, notes) VALUES (?, 'material_received', 'returned', ?, ?)`,
        id, String(req.user.id), `returned to vendor · signed by ${req.user.name}${notes ? ' · ' + notes : ''}`);
    } catch (e) {
      return res.status(500).json({ error: e.message });
    }
    logAuditEvent({
...
router.post('/enquiries/:id/cancel', requirePermission('rental_tools', 'edit'), async (req, res) => {
  try {
    const id = +req.params.id;
    const enquiry = await pg.get(`SELECT * FROM rental_tool_enquiry WHERE id=?`, id);
    if (!enquiry) return res.status(404).json({ error: 'Enquiry not found' });
    ...
    const notes = (req.body?.notes || '').trim();
    await pg.run(`UPDATE rental_tool_enquiry SET status='cancelled', updated_at=CURRENT_TIMESTAMP WHERE id=?`, id);
    await pg.run(`INSERT INTO rental_tool_history (enquiry_id, from_stage, to_stage, triggered_by, notes) VALUES (?, ?, ?, ?, ?)`,
      id, enquiry.current_stage, enquiry.current_stage, String(req.user.id), `cancelled${notes ? ' · ' + notes : ''}`);
    logAuditEvent({
      user: req.user, action: 'UPDATE', entity_type: 'rental_tool_enquiry',
      entity_id: id, entity_label: enquiry.enquiry_no,
...
router.put('/settings/approver', requirePermission('rental_tools', 'edit'), async (req, res) => {
  try {
    if (req.user.role !== 'admin') return res.status(403).json({ error: 'Admin only' });
    const userId = req.body?.user_id ? +req.body.user_id : null;
    if (!userId) {
      await pg.run(`DELETE FROM app_settings WHERE key='rental_approver_user_id'`);
      return res.json({ user_id: null });
    }
    const u = await pg.get(`SELECT id, name FROM users WHERE id=? AND active != 0`, userId);
    if (!u) return res.status(400).json({ error: 'User not found or inactive' });
    await pg.run(`INSERT INTO app_settings (key, value) VALUES (?, ?) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value`,
      'rental_approver_user_id', String(userId));
