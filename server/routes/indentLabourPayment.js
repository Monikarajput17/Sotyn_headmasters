...
// the planned_value).
async function projectRollup(db, projectId) {
  const l1 = num((await db.get(
    `SELECT COALESCE(SUM(amount), 0) AS s FROM proj_salary_entries WHERE project_id = ?`, projectId
  ))?.s);
  const l2 = num((await db.get(
    `SELECT COALESCE(SUM(total_amount), 0) AS s FROM proj_daily_wage_entries WHERE project_id = ?`, projectId
  ))?.s);
  const l3 = num((await db.get(
    `SELECT COALESCE(SUM(amount_paid), 0) AS s FROM proj_work_orders WHERE project_id = ?`, projectId
  ))?.s);
...
// PROJECT CRUD
// ════════════════════════════════════════════════════════════════
router.get('/projects', requirePermission('indent_labour_payment', 'view'), async (req, res) => {
  try {
    ...
    const rows = (await pg.all(sql, ...params)).map(r => ({ ...r, budget: r.l1 + r.l2 + r.l3 }));
    res.json(rows);
  } catch (e) { res.status(500).json({ error: e.message }); }
});

router.get('/projects/:id', requirePermission('indent_labour_payment', 'view'), async (req, res) => {
  try {
    const row = await pg.get(
      `SELECT id, name, COALESCE(owner, 'Aanchal') AS owner, notes, created_at, updated_at
         FROM proj_projects WHERE id = ?`, req.params.id
    );
    if (!row) return res.status(404).json({ error: 'Project not found' });
    res.json({ ...row, ...(await projectRollup(pg, row.id)) });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

router.post('/projects', requirePermission('indent_labour_payment', 'create'), async (req, res) => {
  try {
    const name = String(req.body?.name || '').trim();
    if (!name) return res.status(400).json({ error: 'Project name is required' });
    // Uniqueness — surface as a friendly error instead of the raw
    // SQLITE_CONSTRAINT.  Case-insensitive check.
    const dup = await pg.get(`SELECT id FROM proj_projects WHERE LOWER(TRIM(name)) = LOWER(?)`, name);
    ...
    const r = await pg.run(
      ...
      name, owner, notes, req.user.id
    );
    res.status(201).json({ id: r.lastInsertRowid });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

router.put('/projects/:id', requirePermission('indent_labour_payment', 'edit'), async (req, res) => {
  try {
    const cur = await pg.get(`SELECT id FROM proj_projects WHERE id = ?`, req.params.id);
    ...
    await pg.run(
      `UPDATE proj_projects
        ...
        WHERE id = ?`,
      name, req.body?.owner ?? null, req.body?.notes ?? null, req.params.id
    );
    res.json({ message: 'Updated' });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

router.delete('/projects/:id', requirePermission('indent_labour_payment', 'edit'), async (req, res) => {
  try {
    const cur = await pg.get(`SELECT id FROM proj_projects WHERE id = ?`, req.params.id);
    if (!cur) return res.status(404).json({ error: 'Project not found' });
    // CASCADE on the FKs handles salary / daily-wage / WOs.
    await pg.run(`DELETE FROM proj_projects WHERE id = ?`, req.params.id);
    res.json({ message: 'Deleted' });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// ─── owners list (filter dropdown) ──────────────────────────────
router.get('/owners', requirePermission('indent_labour_payment', 'view'), async (req, res) => {
  try {
...
// L1 SALARY ENTRIES
// ════════════════════════════════════════════════════════════════
router.get('/projects/:pid/salary', requirePermission('indent_labour_payment', 'view'), async (req, res) => {
  try {
...
router.post('/projects/:pid/salary', requirePermission('indent_labour_payment', 'create'), async (req, res) => {
  try {
    const b = req.body || {};
    ...
    if (kind === 'legacy') {
      const exists = await pg.get(
        `SELECT id FROM proj_salary_entries WHERE project_id=? AND kind='legacy'`, req.params.pid
      );
...
router.delete('/salary/:id', requirePermission('indent_labour_payment', 'edit'), async (req, res) => {
  try {
    const r = await pg.run(`DELETE FROM proj_salary_entries WHERE id=?`, req.params.id);
...
// ════════════════════════════════════════════════════════════════
router.get('/projects/:pid/daily-wages', requirePermission('indent_labour_payment', 'view'), async (req, res) => {
  try {
    res.json(await pg.all(
...
router.post('/projects/:pid/daily-wages', requirePermission('indent_labour_payment', 'create'), async (req, res) => {
  try {
    const b = req.body || {};
    ...
    if (kind === 'legacy') {
      const exists = await pg.get(
        `SELECT id FROM proj_daily_wage_entries WHERE project_id=? AND kind='legacy'`, req.params.pid
      );
...
router.delete('/daily-wages/:id', requirePermission('indent_labour_payment', 'edit'), async (req, res) => {
  try {
    const r = await pg.run(`DELETE FROM proj_daily_wage_entries WHERE id=?`, req.params.id);
    if (r.changes === 0) return res.status(404).json({ error: 'Not found' });
    res.json({ message: 'Deleted' });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
// ════════════════════════════════════════════════════════════════
router.get('/projects/:pid/work-orders', requirePermission('indent_labour_payment', 'view'), async (req, res) => {
  try {
                ...
                ELSE LEAST(999,
                  ROUND(
                    (COALESCE((
                      SELECT SUM(dwi.amount) FROM dpr_work_items dwi
                       WHERE dwi.work_order_id = wo.id
                    ), 0) * 100.0 / wo.planned_value)::numeric,
                    1
                  )
                )
...
router.get('/active-work-orders', requirePermission('indent_labour_payment', 'view'), async (req, res) => {
  try {
    res.json(await pg.all(
...
router.get('/work-orders/:id/dpr-items', requirePermission('indent_labour_payment', 'view'), async (req, res) => {
  try {
    res.json(await pg.all(
...
router.post('/projects/:pid/work-orders', requirePermission('indent_labour_payment', 'create'), async (req, res) => {
  try {
    const b = req.body || {};
...
router.put('/work-orders/:id', requirePermission('indent_labour_payment', 'edit'), async (req, res) => {
  try {
    const cur = await pg.get(`SELECT planned_value FROM proj_work_orders WHERE id=?`, req.params.id);
...
router.delete('/work-orders/:id', requirePermission('indent_labour_payment', 'edit'), async (req, res) => {
  try {
    const r = await pg.run(`DELETE FROM proj_work_orders WHERE id=?`, req.params.id);
    if (r.changes === 0) return res.status(404).json({ error: 'Not found' });
    res.json({ message: 'Deleted' });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
